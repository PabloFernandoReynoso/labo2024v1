...
CA_catastrophe_baseline <- function( pmyexp, pinputexps, pserver="local")
{
  ...
  param_local$metodo <- "MachineLearning" # MachineLearning EstadisticaClasica Ninguno #CAMBIO
...
DR_drifting_baseline <- function( pmyexp, pinputexps, pserver="local")
{
  ...
  param_local$metodo <- "rank_cero_fijo" #CAMBIO
...
FE_historia_baseline <- function( pmyexp, pinputexps, pserver="local")
{
  ...
  param_local$lag2 <- TRUE # no me engraso con los lags de orden 2 #CAMBIO
  param_local$lag3 <- TRUE # no me engraso con los lags de orden 3 #CAMBIO

  # baseline
  param_local$Tendencias1$run <- TRUE  # FALSE, no corre nada de lo que sigue
  param_local$Tendencias1$ventana <- 6
  param_local$Tendencias1$tendencia <- TRUE #CAMBIO
  param_local$Tendencias1$minimo <- TRUE #CAMBIO
  param_local$Tendencias1$maximo <- TRUE #CAMBIO
  param_local$Tendencias1$promedio <- TRUE #CAMBIO
  param_local$Tendencias1$ratioavg <- TRUE #CAMBIO
  param_local$Tendencias1$ratiomax <- TRUE #CAMBIO
  ...
  param_local$CanaritosAsesinos$ratio <- 0.5 #CAMBIO
...
TS_strategy_baseline_202109 <- function( pmyexp, pinputexps, pserver="local")
{
  ...
  param_local$final_train <- c(202107, 202106, 202105, 202104, 202103, 202102, 202101, 202012, 202011, 202010, 202009, 202008, 202002, 202001, 201912, 201911, 201910, 201909) #CAMBIO


  param_local$train$training <- c(202105, 202104, 202103, 202102, 202101, 202012, 202011, 202010, 202009, 202008, 202002, 202001, 201912, 201911, 201910, 201909, 201908, 201907) #CAMBIO
  param_local$train$validation <- c(202106)
  param_local$train$testing <- c(202107)

  # undersampling  baseline
  param_local$train$undersampling <- 0.5 #CAMBIO
...
HT_tuning_baseline <- function( pmyexp, pinputexps, pserver="local")
{
  ...
  param_local$lgb_param <- list(
    ...
    learning_rate = c( 0.01, 0.3 ), #CAMBIO
    feature_fraction = c( 0.4, 0.7 ), #CAMBIO
    num_leaves = c( 1000L, 3000L,  "integer" ), #CAMBIO
    min_data_in_leaf = c( 1500L, 3000L, "integer" ) #CAMBIO
  )


  # una Beyesian de Guantes Blancos, solo hace 15 iteraciones
  param_local$bo_iteraciones <- 100 # iteraciones de la Optimizacion Bayesiana #CAMBIO
...
ZZ_final_baseline <- function( pmyexp, pinputexps, pserver="local")
{
  ...
  param_local$kaggle$envios_desde <-  11000L #CAMBIO
  param_local$kaggle$envios_hasta <- 15000L #CAMBIO
...
ZZ_final_semillerio_baseline <- function( pmyexp, pinputexps, pserver="local")
{
  ...
  param_local$kaggle$envios_desde <-  11000L #CAMBI0
  param_local$kaggle$envios_hasta <- 15000L #CAMBIO
  param_local$kaggle$envios_salto <-   500L
...
corrida_baseline_semillerio_202109 <- function( pnombrewf, pvirgen=FALSE )
{
  ...
  DT_incorporar_dataset_baseline( "DT0001-semF001", "competencia_2024.csv.gz")
  CA_catastrophe_baseline( "CA0001-semF001", "DT0001-semF001" )

  DR_drifting_baseline( "DR0001-semF001", "CA0001-semF001" )
  FE_historia_baseline( "FE0001-semF001", "DR0001-semF001" )

  TS_strategy_baseline_202109( "TS0001-semF001", "FE0001-semF001" )

  HT_tuning_baseline( "HT0001-semF001", "TS0001-semF001" )

  # El ZZ depente de HT y TS
  ZZ_final_semillerio_baseline( "ZZ0001-semF001", c("HT0001-semF001","TS0001-semF001") )
...
corrida_baseline_semillerio_202107 <- function( pnombrewf, pvirgen=FALSE )
{
  ...
  TS_strategy_baseline_202107( "TS0002-semF001", "FE0001-semF001" )

  HT_tuning_baseline( "HT0002-semF001", "TS0002-semF001" )

  # El ZZ depente de HT y TS
  ZZ_final_semillerio_baseline( "ZZ0002-semF001", c("HT0002-semF001","TS0002-semF001") )
...
corrida_baseline_semillerio_202109( "basem01F001" )
